...
class Schema:
    ...
    attributes = [('mean_temperature', 'deg_celsius'), ('minimum_temperature', 'deg_celsius'),
                  ('maximum_temperature', 'deg_celsius'), ('sunshine', 'hour'),
                  ('rain_days', 'day')]
...
class DescribeYear(Schema):
    ...
    def instantiate(self, kb):
        ...
        for attribute, unit in self.attributes:
            ...
            if value:
                template_name = 'report-temperature' if unit in (
                    'deg_celsius',
                ) else 'report-duration'
                report_value = messages.ReportValue(
                    self.year, attribute, value, units=unit, template_name=template_name
                )
